...
class SoQuest:
    def __init__(self, user_id: int, address: str, signature: str, loop=None):
    ...
    async def __dump_xlsx(self) -> str:
        user_dir = BASE_DIR / 'files' / str(self.user_id)
        create_dir(user_dir)

        cur_datetime = datetime.datetime.utcnow().strftime('%d_%m_%Y_%H_%M_%S')
        filename = os.path.join(str(user_dir), f'result_{cur_datetime}.xlsx')
